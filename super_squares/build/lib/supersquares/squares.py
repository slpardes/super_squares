...
class Squares:
    ...
    The number of rows and columns default to ten to create a 10x10 grid. 
    It is optional to set the minimum or maximum number of squares a player can purchase. 
    
    By default, Team A (rows) will be the AFC and Team B (columns) will be the NFC. These values can be overwritten. 
    
    The adjustable cost per square is $5.00.
    
    The payout percentage are set to be 15% for quarters 1 and 3, 30% for quarter 2 (halftime), and 40% of the pot for quarter 4 (the final score). 
    ...
    def __init__(
        ...
        if (q1 + q2 + q3 + q4) != 100:
            print("Error: Quarter payouts must add up to 100%.")

    def square_stats(self):
        """
        The square_stats() function prints information on a created Squares instance.
        ...
        Unless otherwise specified, the team associated with the number of rows is the AFC. 
        The NFC team aligns with the columns.
    ...
    def clear_squares(self):
        ...
        # reset the assigned squares grid and reset the pool of squares
        self.squares_pool = {
            (i, j): None for i in range(self.n_rows) for j in range(self.n_cols)
        }
        self.squares_dict = {}
        return
